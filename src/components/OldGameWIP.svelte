...
<script>
	...
	function start() {
		bg = rgbaStringToHex(level.background)
		console.log(bg)
	...
	function create() {
		// add texture for all blocks in level
		...
		// add player to level
		player = this.physics.add.sprite(0, 600, 'player.still')
	...
	function update() {
		...
		if (cursors.left.isDown && !cursors.right.isDown) {
			player.setVelocityX(-character.maxVelocity)
			player.setScale(-1, 1)
			// todo only play if player.moving is there
			player.anims.play('player.moving.animation', true)
			...
			player.setTexture('player.still')
		}
		...
		// jumping only if you're on ground, or if player can fly
		if (cursors.up.isDown || character.canFly) {
	...
	// 		width: $project.art[character....].width * artScale, // width of graphic
	// 		height: $project.art[character....].height * artScale, // height of graphic
	...
	// 		const w = $project.art[template....].width * artScale // width of graphic
	// 		const h = $project.art[template....].height * artScale // height of graphic
