        ...
        <FieldCharacterPicker name="playableCharacters" bind:value={input.playableCharacters}>
          ...
          <div class="flex g1">
            {#each input.playableCharacters as characterId}
              <a
                href="#/{encodeURIComponent($project.name)}/play/{encodeURIComponent(input.id)}/{encodeURIComponent(characterId)}"
                class="text-success"><Icon data={playIcon} /> Play as {$project.characters[characterId].name}</a
              >
            {/each}
          </div>
