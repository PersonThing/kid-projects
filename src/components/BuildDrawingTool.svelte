        ...
        <div class="flex">
          ...
          <div class="ml-1">
            ...
            <div class="small">
              ...
              {#if option.teleportOnTouch}, teleports on touch{/if}
              ...
              {#if option.consumable}
                ...
                {#if option.followerOnConsume?.length}, spawns <strong
                    >{option.followerOnConsume.map(f => $project.characters[f].name).join(', ')}</strong
                  > followers on consume{/if}
                {#if option.enemyOnConsume?.length}, spawns <strong>{option.enemyOnConsume.map(e => $project.enemies[e].name).join(', ')}</strong> enemies
                  on consume{/if}
              {/if}
            </div>
          </div>
...
<script>
  ...
  function placeItem(x, y) {
    ...
    if (selectedBlock != null) {
      blocks = stripOutliersAndSort([...blocks, [selectedBlock, x, y]])
    } else if (selectedEnemy != null) {
      enemies = stripOutliersAndSort([...enemies, [selectedEnemy, x, y]])
    }
  }
